...
JSON_SCHEMA_FILE := project/jsonschema/minimal_metab.schema.json
...
run-linkml-validation: src/minimal_metab/schema/minimal_metab.yaml \
src/data/examples/invalid/SampleOperationsCollection-undefined-slot.yaml
	# SampleOperationsCollection is assumed as the target-class because it has been defined as the tree_root in the schema
	- $(RUN) linkml-validate \
	  --schema $^


src/data/dh_vs_linkml_json/SampleOperationsCollection_linkml_raw.yaml: src/data/dh_vs_linkml_json/SampleOperations_dh.json
	$(RUN) dh-json2linkml \
		--input-file $< \
		--output-file $@ \
		--output-format yaml \
...
src/data/dh_vs_linkml_json/SampleOperationsCollection_linkml_normalized.yaml: src/data/dh_vs_linkml_json/SampleOperationsCollection_linkml_raw.yaml
	$(RUN) linkml-normalize \
		--schema src/minimal_metab/schema/minimal_metab.yaml \
		--output $@ \
		--no-expand-all $<

src/data/dh_vs_linkml_json/entries.json: src/data/dh_vs_linkml_json/SampleOperationsCollection_linkml_normalized.yaml
	$(RUN) linkml-json2dh \
		--input-file $< \
		--input-format yaml \
		--output-dir $(dir $@)
