...
all: site
site: gen-project gendoc project/json/minimal_metab.json
%.yaml: gen-project
...
gen-examples:
	cp src/data/examples/valid/SampleOperationsCollection*.yaml $(EXAMPLEDIR)
...
examples/output: src/minimal_metab/schema/minimal_metab.yaml
	mkdir -p $@
...
git-init:
	git init
git-add: .cruft.json
	git add .gitignore .github .cruft.json Makefile LICENSE *.md examples utils about.yaml mkdocs.yml poetry.lock project.Makefile pyproject.toml src/minimal_metab/schema/*yaml src/*/datamodel/*py src/data src/docs tests src/*/_version.py project/reports/ src/local_schemasheets/ data_harmonizer/
